...
import { wrapSvc } from '../src/infra/usage';
...
class EventBus {
  ...
  on = wrapSvc('svc:services/eventBus#on', (event: string, listener: Listener): () => void => {
    if (!this.listeners.has(event)) this.listeners.set(event, new Set());
    this.listeners.get(event)!.add(listener);
    return () => this.off(event, listener);
  });

  off = wrapSvc('svc:services/eventBus#off', (event: string, listener: Listener): void => {
  ...
  emit = wrapSvc('svc:services/eventBus#emit', (event: string, payload?: any): void => {
    const set = this.listeners.get(event);
    if (!set) return;
